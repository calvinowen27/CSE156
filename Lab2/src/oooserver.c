	...
	while ((bytes_recvd = recvfrom(sockfd, buf[packets_recvd], BUFFER_SIZE, 0, sockaddr, sock_size)) >= 0) {
		if ((uint8_t)buf[packets_recvd][1] == serving_client_id) {
			if (bytes_recvd > HEADER_SIZE) { // received good packet, echo back
				...
				if (packets_recvd == 5) {
					for (int i = 4; i >= 0; i--) {
						if (sendto(sockfd, buf[i], BUFFER_SIZE, 0, sockaddr, *sock_size) < 0) {
				...
				for (int i = packets_recvd-1; i >= 0; i--) {
					if (sendto(sockfd, buf[i], BUFFER_SIZE, 0, sockaddr, *sock_size) < 0) {
						fprintf(stderr, "myserver ~ main(): server failed to send packets back to client.\n");
						break;
					}
			...
			if (waiting_for_client) {
				buf[packets_recvd][1] = next_client_id;
				serving_client_id = next_client_id;
				waiting_for_client = 0;
				next_client_id += 1;
			} else {
				// printf("server recv packet from wrong client, turning away\n");
				buf[packets_recvd][HEADER_SIZE] = 0;
			}

			if (sendto(sockfd, buf[packets_recvd], BUFFER_SIZE, 0, sockaddr, *sock_size) < 0) {
				fprintf(stderr, "myserver ~ main(): server failed to send packets back to client.\n");
				break;
			}
		}
